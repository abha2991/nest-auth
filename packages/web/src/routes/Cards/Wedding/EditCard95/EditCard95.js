...
import weddingCard1 from '../../../img/Wedding/Wedding_22_1.png'
import weddingCard2 from '../../../img/Wedding/Wedding_22_2.png'
...
const EditCard95 = () => {
  ...
  if (textdata) {
    FirstPageData = ['name1', 'name2', textdata[0]?.name1 ?? '', textdata[0]?.name2 ?? '']
  ...
  const PostData = async (e) => {
    ...
    details = [
      { name1, weds, name2 },
      { _name1, _with, _name2, date, time, venue }
    ]

    let card_id = cardData.cardId
    setLoading(true)
    const res = await fetch(`http://localhost:3001/api/card1/weddingcard16`, {
      method: 'POST',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json'
