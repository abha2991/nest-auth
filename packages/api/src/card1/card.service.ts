...
export class CardService {
  ...
  async WeddingCard(@Body() cardDto: CardDto) {
    const generatePreviewImage = async (id, Details, userid, email, maxCharsPerLine) => {
      try {
        ...
        for (k = 0; k < details.noOfPages; k++) {
          try {
            image = await Jimp.read(
              resolve('src', 'card1', 'assets', details.cardCategory, details.Caption[k].previewPage)
            )

            const capt1 = await details.Caption[0].Caption[0]
            let y_value = 0
            const secondPageCaption = (await details.Caption[1].Caption[0]?.y) ?? ''
            y_value = y_value + secondPageCaption

            const secondPageCaption1 = (await details.Caption[1].Caption[1]?.y) ?? ''
                ...
                for (let e = 0; e < text.length; e++) {
                  ...
                  if (k === 1) {
                    ...
                    if (i === 0) {
                      ...
                      const singleLineHeight1 = Jimp.measureTextHeight(font, 'a', textWidth)
                      if (text[0] === 'Weds') {
                        y_value1 = y_value1 + singleLineHeight1 + 20
                      }
                      await image.print(
                        font,
                      ...
                      if (text[0] === 'Weds') {
                        y_value1 = y_value1 + singleLineHeight1 + 100
                      } else {
                        y_value1 = y_value1 + singleLineHeight1 + 10
                      }
                    }
                  } else if (k === 2) {
        ...
        let createdCardId = data?.id ?? ''
        return { createdCardId, status: 'Success', message: 'Successfully Created' }
      } catch (e) {
        throw new HttpException(
          {
            statusCode: HttpStatus.BAD_REQUEST,
                ...
                for (let e = 0; e < text.length; e++) {
                  ...
                  if (k === 2) {
                    ...
                    if (i === 0 && e === 0) {
                      ...
                      const singleLineHeight = Jimp.measureTextHeight(font, 'a', textWidth)
                      fourthPageCaption = fourthPageCaption + singleLineHeight + 20
                    } else if (i === 4 && e === 0) {
                      const singleLineHeight = Jimp.measureTextHeight(font, 'a', textWidth)
  ...
  async WeddingCard16(@Body() cardDto: CardDto) {
    const generatePreviewImage = async (id, Details, userid, email, maxCharsPerLine) => {
      try {
        let fileName = []
        var image
                ...
                for (let e = 0; e < text.length; e++) {
                  ...
                  if (k == 0) {
                    ...
                    if (i === 0 && e === 0) {
                      ...
                      const singleLineHeight = Jimp.measureTextHeight(font, 'a', textWidth)
                      let space = singleLineHeight + spaceBetweenLines * 9
      ...
      try {
        ...
        for (k = 0; k < details.noOfPages; k++) {
          try {
            ...
            const capt2 = await details.Caption[0].Caption[3]
            ...
            for (let i = 0; i < details.Caption[k].Caption.length; i++) {
              ...
              if (data[i]) {
                ...
                for (let e = 0; e < text.length; e++) {
                  ...
                  if ((i === 1 || i === 2) && e === 0) {
                    let singleLineHeight = Jimp.measureTextHeight(font, 'a', textWidth)
                    y = y + 140
                  }
                  if ((i === 1 || i === 2 || i === 0) && e != 0) {
                    let singleLineHeight = Jimp.measureTextHeight(font, 'a', textWidth)
      ...
      try {
        ...
        for (k = 0; k < details.noOfPages; k++) {
          try {
            ...
            let y = 0
            y = y + capt1?.y ?? ''
            const capt2 = await details.Caption[0].Caption[1]

            let y1 = capt2?.y ?? ''

            const capt3 = await details.Caption[1].Caption[0]

            let y2 = capt3?.y ?? ''

            const capt4 = await details.Caption[1].Caption[2]
      ...
      try {
        ...
        for (k = 0; k < details.noOfPages; k++) {
          try {
            ...
            const capt3 = await details.Caption[1].Caption[1]
            ...
            for (let i = 0; i < details.Caption[k].Caption.length; i++) {
              ...
              if (data[i]) {
                ...
                let text
                if (i === 0 && k === 0) {
                  if (a.length > maxCharsPerLine - 5) {
                    text = breakTextToLines(data[i] as string, maxCharsPerLine - 5, 1)
                  } else {
                    text = [data[i]]
                  }
                } else if ((i === 1 || i === 2) && k === 0) {
  ...
  async ReceptionCard4(@Body() cardDto: CardDto) {
    const generatePreviewImage = async (id, Details, userid, email, maxCharsPerLine) => {
      let fileName = []
      var image
      const details = await this.cardetailsService.find(id)
  ...
  async BirthdayCard2(@Body() cardDto: CardDto) {
    const generatePreviewImage = async (id, Details, userid, email, maxCharsPerLine) => {
      try {
        let fileName = []
        var image
